...
svn_revision = "".join(filter(str.isdigit, "$Revision: 165 $"))
...
# Public use methods
def setParseMode(mode):
    """ Use to choose which parser is used. Options are "fast" and "sans". "sans" will use the SANS parser which has better error messages (but which is very slow on certain files). Setting it to the default, "fast", will use the built-in fast parser which doesn't have as nice of error messages."""

    global parse_mode

    if mode != "fast" and mode != "sans":
        raise ValueError("Parse mode: " + mode + " not supported. Please use parse mode 'fast' or 'sans'.")
    elif mode == "fast":
        parse_mode = "fast"
    elif mode == "sans":
        parse_mode = "sans"
...
class _fastParser():
    ...
    def indexHandle(self, haystack, needle, startpos=None):
        try:
    ...
    def realgetToken(self):
        ...
        while len(tmp) == 0:
            ...
            try:
                raw_tmp = self.full_data[self.index:self.full_data.index("\n",self.index+1)]
            except ValueError:
                # End of file
        ...
        if tmp.startswith(";\n"):
            ...
            if "\n;" in tmp:
                ...
                # The line was terminated improperly
                    ...
                    if self.nextWhitespace(tmp[until+2:]) == 0:
                        if raise_parse_warnings:
                            raise ValueError("Warning: Technically invalid line found in file. Multiline values should terminate with \\n;\\n but in this file only \\n; with non-return whitespace following was found.", self.getLineNumber() )
                        self.token = tmp[0:until+1]
                        self.index += until + 4
                        self.last_delineator = ";"
                        return
                ...
                raise ValueError("Invalid file. Multi-line comment never ends. Multi-line comments must terminate with a line that consists ONLY of a ';' without characters before or after. (Other than the newline.)", self.getLineNumber())
        ...
        if tmp.startswith("'"):
            ...
            if until is None:
                raise ValueError("Invalid file. Single quoted value was never terminated.", self.getLineNumber())
        ...
        if tmp.startswith('"'):
            ...
            if until is None:
                raise ValueError("Invalid file. Double quoted value was never terminated.", self.getLineNumber())
    ...
    def parse(self, data):
        ...
        # Knowing there is always at least one newline makes the parsing code simpler
        self.full_data = data + "\n"
        ...
        if not self.token.startswith("data_"):
            raise ValueError("Invalid file. NMR-STAR files must start with 'data_'. Did you accidentally select the wrong file?", self.getLineNumber())

        # Make sure there is a data name
        elif len(self.token) < 6:
            raise ValueError("'data_' must be followed by data name. Simply 'data_' is not allowed.", self.getLineNumber())
        ...
        # We are expecting to saveframes
        while self.getToken() != None:

            if not self.token.startswith("save_"):
                raise ValueError("Only 'save_NAME' is valid in the body of a NMR-STAR file. Found '" + self.token + "'.", self.getLineNumber())

            if len(self.token) < 6:
                raise ValueError("'save_' must be followed by saveframe name. You have a 'save_' tag which is illegal without a specified saveframe name.", self.getLineNumber())

            if self.last_delineator != "":
                raise ValueError("The save_ keyword may not be quoted or semicolon-delineated.", self.getLineNumber())

            # Add the saveframe
            curframe = saveframe.fromScratch(self.token[5:])
            ...
            while self.getToken() != None:
                ...
                if self.token == "loop_":
                    if self.last_delineator != "":
                        raise ValueError("The loop_ keyword may not be quoted or semicolon-delineated.", self.getLineNumber())

                    curloop = loop.fromScratch()
                    ...
                    while in_loop and self.getToken() != None:
                        ...
                        if self.token.startswith("_"):
                            if self.last_delineator != "":
                                raise ValueError("Loop tags may not be quoted or semicolon-delineated.", self.getLineNumber())
                            if seen_data == True:
                        ...
                        # On to data
                            ...
                            while self.token != None:
                                if self.token == "stop_":
                                    if self.last_delineator != "":
                                        raise ValueError("The stop_ keyword may not be quoted or semicolon-delineated.", self.getLineNumber())
                                    if len(curloop.columns) == 0:
                                        if raise_parse_warnings:
                                            raise ValueError("Loop with no tags.", self.getLineNumber())
                                        curloop = None
                                    elif len(curloop.data) == 0 and raise_parse_warnings:
                                        raise ValueError("Loop with no data.", self.getLineNumber())
                                    else:
                                        curloop._autoAddData(curdata)
                                        curloop = None
                                        curdata = []
                                    ...
                                    if len(curloop.columns) == 0:
                                        raise ValueError("Data found in loop before loop tags.", self.getLineNumber())

                                    if self.token in self.reserved and self.last_delineator == "":
                                        raise ValueError("Cannot use keywords as data values unless quoted or semi-colon delineated. Perhaps this is a loop that wasn't properly terminated? Illegal value: " + self.token, self.getLineNumber())
                    ...
                    if self.token != "stop_":
                        raise ValueError("Loop improperly terminated at end of file.", self.getLineNumber())
                ...
                # Invalid content in saveframe
                    ...
                    raise ValueError("Invalid token found in saveframe '" + curframe.name +  "': '" + self.token + "'", self.getLineNumber())

                # Add a tag
                else:
                    if self.last_delineator != "":
                        raise ValueError("Saveframe tags may not be quoted or semicolon-delineated.", self.getLineNumber())
                    curtag = self.token

                    # We are in a saveframe and waiting for the saveframe tag
                    self.getToken()
                    if self.token in self.reserved and self.last_delineator == "":
                        raise ValueError("Cannot use keywords as data values unless quoted or semi-colon delineated. Illegal value: " + self.token, self.getLineNumber())
                    curframe.addTag(curtag, self.token)

            if self.token != "save_":
                raise ValueError("Saveframe improperly terminated at end of file.", self.getLineNumber())
...
# Internal use class
class _sansParser(ContentHandler, ErrorHandler):
...
class entry:
    ...
    def __init__(self, **kargs):
        ...
        if 'the_string' in kargs:
		    ...
		    star_buffer = StringIO(urlopen('http://www.bmrb.wisc.edu/ftp/pub/bmrb/metabolomics/NMR_STAR_experimental_entries/%s.str' % (kargs['entry_num']).read().decode()))
                else:
                    #star_buffer = urlopen('http://rest.bmrb.wisc.edu/bmrb/NMR-STAR3/%s' % kargs['entry_num'])
		    star_buffer = urlopen('http://www.bmrb.wisc.edu/ftp/pub/bmrb/metabolomics/NMR_STAR_experimental_entries/%s.str' % (kargs['entry_num']))
            except HTTPError:
        ...
        if parse_mode == "sans":
            t = _sansParser(entry=self)
            SansParser.parser(STARLexer(star_buffer), t, t).parse()
        else:
            parser = _fastParser(entry=self)
...
class saveframe:
    ...
    def __init__(self, **kargs):
        ...
        if parse_mode == "sans":
            ...
            t = _sansParser(entry=tmp_entry)
            SansParser.parser(STARLexer(star_buffer), t, t).parse()
        else:
            star_buffer = StringIO("data_1 " + star_buffer.read() )
            parser = _fastParser(entry=tmp_entry)
            parser.parse(star_buffer.read())
...
class loop:
    ...
    def __init__(self, **kargs):
        ...
        if parse_mode == "sans":
            ...
            star_buffer = StringIO("data_0 save_internaluseyoushouldntseethis_frame " + star_buffer.read() + " save_")
            parser = _fastParser(entry=tmp_entry)
            parser.parse(star_buffer.read())
    ...
    def _autoAddData(self, the_list):
        """ An internal use method that breaks loop data into appropriate sized chunks automatically while parsing. """
        # Break their data into chunks based on the number of columns
        processed_data = [the_list[x:x + len(self.columns)] for x in xrange(0, len(the_list), len(self.columns))]
        if len(processed_data[-1]) != len(self.columns):
            raise ValueError("The number of data elements in the loop " + self.category + " does not match the number of columns!")
